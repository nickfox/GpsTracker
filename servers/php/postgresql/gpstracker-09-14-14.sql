...
CREATE TABLE gpslocations (
  ...
  gpsTime timestamp NOT NULL DEFAULT '0001-01-01 00:00:00',
...
(4,'0001-01-01 00:00:00',47.593757,-122.195074,'gpsTracker2','gpsTracker2','8BA21D90-3F90-407F-BAAE-800B04B1F5EC',0,0,0.0,'2007-01-03 19:40:00','na',137,'na','gpsTracker'),
...
CREATE OR REPLACE VIEW v_getallroutesformap AS 
 SELECT gpslocations.sessionid,
   ...
   FROM ( SELECT max(gpslocations_1.gpslocationid) AS id
          ...
          WHERE gpslocations_1.sessionid::text <> '0'::text AND char_length(gpslocations_1.sessionid::text) <> 0 AND gpslocations_1.gpstime <> '0001-01-01 00:00:00'::timestamp without time zone
...
SELECT 
  ...
  FROM gpslocations
  WHERE gpstime != '0001-01-01 00:00:00'
  ORDER BY lastupdate;
 ;
